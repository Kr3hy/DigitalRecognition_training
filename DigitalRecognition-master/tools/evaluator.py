...
#     import numpy as np
#     print(f"Testing {global_config.MODEL_NAME}...\n")
...
#
#     precs = {a: np.empty(0) for a in classes.CLASSES_NAME.keys()}
#     recs = {a: np.empty(0) for a in classes.CLASSES_NAME.keys()}
#     threshold_loop = tqdm(np.arange(0.4, 1.0, 0.05))
#     for threshold in threshold_loop:
#         tp = {a: 0 for a in classes.CLASSES_NAME.keys()}
#         fp = {a: 0 for a in classes.CLASSES_NAME.keys()}
...
#         for cls in classes.CLASSES_NAME.keys():
#             precs[cls] = np.append(precs[cls], prec[cls])
#             recs[cls] = np.append(recs[cls], rec[cls])
#
#     import numpy as np
#     ap = {a: 0.0 for a in classes.CLASSES_NAME.keys()}
#     map = 0.0
#     for cls in classes.CLASSES_NAME.keys():
...
#     for cls in classes.CLASSES_NAME:
    ...
    for threshold in threshold_loop:
        ...
        for imgs, labels in test_loop:
            ...
            for idx, label in enumerate(labels):
                ...
                # print(labels)
                # print("end")
                index=int(labels.argmax())
                # print(index)
                if pred[-1][idx] == label and pred[0][idx] >= threshold:
                    # print(tp)
                    # print(idx+1)
                    tp[index+1] += 1 # data type problem
                else:
                    # print(tp)
                    # print(idx+1)
                    fp[index+1] += 1
